...
sensorPosition1, lightIntensity1 = np.loadtxt(
...
plt.plot(
    sensorPosition1,
    cosineModel(sensorPosition1, popt1[0]),
    color="red",
...
plt.legend()
plt.savefig(f"{directory}/../figures/figure4.pdf")
plt.cla()

plt.errorbar(
    np.cos(sensorPosition1) ** 2,
    lightIntensity1,
    yerr=0.01,
    xerr=(
        2
        * (abs(np.sin(sensorPosition1)) * 0.00001)
        / np.cos(sensorPosition1) ** 2
    ),
    color="blue",
    fmt="o",
...
plt.ylabel("Light Intensity (V)")
plt.legend()
plt.savefig(f"{directory}/../figures/figure5.pdf")
plt.cla()

plt.scatter(
    sensorPosition1,
    cosineModel(sensorPosition1, popt1[0]) - lightIntensity1,
    label="Residual Plot for the Intensity vs Angle",
)
plt.xlabel("Angle (rad)")
plt.ylabel("Light Intensity Difference (V)")
plt.legend()
plt.savefig(f"{directory}/../figures/figure6.pdf")
plt.cla()

plt.scatter(
    np.cos(sensorPosition1) ** 2,
    cosineModel(sensorPosition1, popt1[0]) - lightIntensity1,
    label="Residual Plot for the Intensity vs the "
    "Cosine of the Angle Squared",
)
plt.xlabel("Cosine of Angle Squared")
plt.ylabel("Light Intensity Difference (V)")
plt.legend()
plt.savefig(f"{directory}/../figures/figure7.pdf")
plt.cla()
...
sensorPosition2, lightIntensity2 = np.loadtxt(
    f"{directory}/rawData/experimentTwo.txt",
    delimiter="\t",
    skiprows=2,
    unpack=True,
...
print(
    "The intensity passing through the first polarizer is "
    + str(popt2[0])
    + " ± "
    + str(pvar2[0])
)
...
plt.plot(
    sensorPosition2,
    intensity(sensorPosition2, popt2[0]),
    color="red",
    label="Theoretical Model",
)
plt.xlabel("Angle (rad)")
plt.ylabel("Light Intensity (V)")
plt.savefig(f"{directory}/../figures/figure8.pdf")
plt.cla()

plt.scatter(
    sensorPosition2,
    intensity(sensorPosition2, popt2[0]) - lightIntensity2,
    label="Residual Plot for the Intensity vs Angle",
)
plt.xlabel("Angle (rad)")
plt.ylabel("Light Intensity Difference (V)")
plt.legend()
plt.savefig(f"{directory}/../figures/figure9.pdf")
plt.cla()
...
print(
    characterize(lightIntensity1, cosineModel(sensorPosition1, popt1[0]), 0.01)
)
